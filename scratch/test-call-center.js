...
async function main() {
  ...
  console.log('--- Testing parsePageContent for app/services/call-center/page.js ---');
  ...
  await scanRoutes();
  const routeDoc = await db.collection('cms_routes').findOne({ path: '/services/call-center' });
  console.log('\n--- Route doc in DB for /services/call-center ---', routeDoc);
