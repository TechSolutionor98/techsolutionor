...
async function syncHandymanDb() {
  ...
  try {
    ...
    const serviceSlug = 'handyman-services';
    ...
    if (existingContent) {
      ...
      console.log('Successfully updated cms_page_content in DB for /services/handyman-services');
    } else {
      updateDoc.createdAt = now;
      updateDoc.version = 1;
      await db.collection('cms_page_content').insertOne(updateDoc);
      console.log('Successfully inserted cms_page_content in DB for /services/handyman-services');
    }
  } catch (err) {
