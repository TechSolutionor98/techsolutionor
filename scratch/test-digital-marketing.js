...
async function main() {
  const filePath = path.join(process.cwd(), 'app/services/digital-marketing/page.js');
  console.log('--- Testing parsePageContent for app/services/digital-marketing/page.js ---');
  ...
  await scanRoutes();
  const routeDoc = await db.collection('cms_routes').findOne({ path: '/services/digital-marketing' });
  console.log('\n--- Route doc in DB for /services/digital-marketing ---', routeDoc);
