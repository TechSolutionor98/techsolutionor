...
function testParsingV4(filePath) {
  ...
  // 2. Innermost DIV tags (badge texts, section labels)
  const divRegex = /<div\b([^>]*)>([\s\S]*?)<\/div>/gi;
  let divIdx = 1;
  while ((match = divRegex.exec(cleanContent)) !== null) {
    const innerContent = match[2];

    // Ensure it doesn't contain any nested elements or block tags
...
files.forEach(f => {
  ...
  console.log('\n--- ' + f + ' ---');
  console.log(JSON.stringify(testParsingV4(p), null, 2));
});
