...
async function main() {
  const filePath = path.join(process.cwd(), 'app/services/social-media/page.js');
  console.log('--- Testing parsePageContent for app/services/social-media/page.js ---');
  ...
  await scanRoutes();
  const routeDoc = await db.collection('cms_routes').findOne({ path: '/services/social-media' });
  console.log('\n--- Route doc in DB for /services/social-media ---', routeDoc);
