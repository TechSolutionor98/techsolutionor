...
async function main() {
  const filePath = path.join(process.cwd(), 'app/services/ppc-amazon-ads/page.js');
  console.log('--- Testing parsePageContent for app/services/ppc-amazon-ads/page.js ---');
  ...
  await scanRoutes();
  const routeDoc = await db.collection('cms_routes').findOne({ path: '/services/ppc-amazon-ads' });
  console.log('\n--- Route doc in DB for /services/ppc-amazon-ads ---', routeDoc);
