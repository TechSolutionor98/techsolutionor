...
let newLines = [];
let stack = [];
let passedAcWork = false;
let skipNextClosingDiv = false;

for (let i = 0; i < lines.length; i++) {
    let line = lines[i];

    if (line.includes('if (isElectricalWork) {')) {
        passedAcWork = true;
    }
    ...
    if (line.includes('lg:w-1/2') && !line.includes('space-y-6') && !line.includes('bg-white') && !line.includes('min-h-full')) {
        ...
        if (i + 1 < lines.length && lines[i+1].includes('aspect-[4/3] rounded-3xl')) {
    ...
    // Safely remove inner image wrappers
    if (line.includes('relative aspect-[4/3] rounded-3xl')) {
       skipNextClosingDiv = true;
       continue;
    }
    if (line.trim() === '</div>' && skipNextClosingDiv) {
...
console.log('Applied formatting line by line safely fixed.');
