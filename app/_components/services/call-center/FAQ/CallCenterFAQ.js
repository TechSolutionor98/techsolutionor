...
const defaultFaqscallcenterquestion = [
  {
...
export default function CallCenterFAQComponent({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "callcenterfaq");
  const faqs = defaultFaqscallcenterquestion.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "callcenterfaq"),
    answer: getCmsVal(cmsContent, item.answer, "callcenterfaq"),
  }));

  return <CallCenterFAQ title={title} faqs={faqs} />;
}
