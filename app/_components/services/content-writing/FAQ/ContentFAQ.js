...
const defaultFaqscontentwritingquestion = [
  {
...
export default function ContentFAQ({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "contentfaq");
  const faqs = defaultFaqscontentwritingquestion.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "contentfaq"),
    answer: getCmsVal(cmsContent, item.answer, "contentfaq"),
  }));

  return <ContentWritingFAQ title={title} faqs={faqs} />;
}
