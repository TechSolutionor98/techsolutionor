...
export default function SEOFAQ({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "seofaq");
  const faqs = defaultFaqsscoquestion.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "seofaq"),
    answer: getCmsVal(cmsContent, item.answer, "seofaq"),
  }));

  return <SeoFAQ title={title} faqs={faqs} />;
}
