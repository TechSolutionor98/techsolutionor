...
export default function TechFAQ({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "techfaq");
  const faqs = defaultFaqstechquestion.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "techfaq"),
    answer: getCmsVal(cmsContent, item.answer, "techfaq"),
  }));
