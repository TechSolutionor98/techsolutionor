...
const defaultFaqsseoauditquestions = [
  {
...
export default function SeoAuditFAQComponent({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "seoauditfaq");
  const faqs = defaultFaqsseoauditquestions.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "seoauditfaq"),
    answer: getCmsVal(cmsContent, item.answer, "seoauditfaq"),
  }));

  return <SeoAuditFAQ title={title} faqs={faqs} />;
}
