...
export default function TechFAQS({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "laraveltechfaqs");
  const faqs = defaultFaqstechquestion.map((faq) => ({
    question: getCmsVal(cmsContent, faq.question, "laraveltechfaqs"),
    answer: getCmsVal(cmsContent, faq.answer, "laraveltechfaqs"),
  }));
