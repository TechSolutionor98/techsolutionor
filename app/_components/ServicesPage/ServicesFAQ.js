...
const defaultFaqsservicesquestion = [
  {
...
export default function ServicesFAQ({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "servicesfaq");
  const faqs = defaultFaqsservicesquestion.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "servicesfaq"),
    answer: getCmsVal(cmsContent, item.answer, "servicesfaq"),
  }));
