...
export default function POSFAQComponent({ cmsContent }) {
  const title = getCmsVal(cmsContent, "Frequently Asked Questions (FAQs)", "posfaq");
  const faqs = defaultFaqsposquestions.map((item) => ({
    question: getCmsVal(cmsContent, item.question, "posfaq"),
    answer: getCmsVal(cmsContent, item.answer, "posfaq"),
  }));

  return <POSFAQ title={title} faqs={faqs} />;
}
