...
let isSyncing = false;
let lastSyncTime = 0;
const MIN_INTERVAL_MS = 60000; // at least 60 seconds between sync checks
...
export async function triggerAutoSync({ waitForCompletion = false, limit = 25, markSeen = true } = {}) {
  ...
  const syncPromise = (async () => {
    try {
      const result = await syncIncomingEmails({ limit, markSeen, onlyUnseen: true });
...
// Background poller interval in Node runtime (runs every 2 minutes when server is running)
