      ...
      updateOne: {
        ...
        update: {
          $set: {
            ...
            filePath: r.filePath,
            parentPath: r.parentPath !== undefined ? r.parentPath : (r.path.split('/').filter(Boolean).length > 1 ? '/' + r.path.split('/').filter(Boolean).slice(0, -1).join('/') : (r.path.split('/').filter(Boolean).length === 1 ? '/' : null)),
            depth: r.depth !== undefined ? r.depth : r.path.split('/').filter(Boolean).length,
            websiteId: 'default',
            lastScanned: now,
            updatedAt: now,
          },
