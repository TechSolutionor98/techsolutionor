...
export async function scanRoutes() {
  ...
  function scanDir(dirPath, routePrefix = '') {
    ...
    for (const item of items) {
      if (item.name.startsWith('_') || item.name.startsWith('.') || item.name === 'api' || item.name === 'admin' || item.name.toLowerCase() === 'home' || (item.name === '[slug]' && routePrefix === '')) {
        continue;
      }
  ...
  // Purge legacy src/app/ duplicate records, duplicate /Home route records, root [slug] catch-all records, and stale non-existent files
...
export async function getRoutesList() {
  ...
  return routes
    .filter(r => r.path !== '/[slug]' && r.status !== 'archived')
    .map(r => ({
      ...r,
      _id: r._id.toString(),
    }));
