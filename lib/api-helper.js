...
function extractFilename(src) {
  ...
  // Next.js hashed file format in production/build: name.[hash].[ext] e.g. "swift2.ac3ede4a.png"
  const parts = name.split('.');
  if (parts.length > 2 && /^[a-f0-9]+$/i.test(parts[parts.length - 2])) {
    return parts.slice(0, -2).join('.').toLowerCase().replace(/[-_]/g, '');
  }
...
export function isValidImageUrl(val) {
  ...
  const s = val.trim();
  if (s.startsWith('@/')) return false; // Code import alias, not a valid browser/Next URL
  if (s.startsWith('/') || /^https?:\/\//i.test(s) || s.startsWith('data:image/')) return true;
  return false;
}
...
function isImageMatch(field, originalValue) {
  ...
  // Exact filename match only! Never do partial .includes because unrelated images (e.g. "laravel" logo vs "laravelshowcase" banner) collide!
  if (fieldFile && origFile && fieldFile === origFile) {
    return true;
  }
...
export function getCmsVal(contentInput, originalValue, sectionIdentifier) {
  let val = originalValue;
  const content = contentInput?.content || contentInput;
  ...
  if (content) {
    ...
    if (content.fields && typeof content.fields === 'object') {
      for (const [key, field] of Object.entries(content.fields)) {
        if (!field) continue;
        if (isImageReq) {
          if (isImageMatch(field, originalValue) || key === originalValue || field.label === originalValue) {
            if (isValidImageUrl(field.value)) {
              return field.value;
            }
          }
        } else {
          if (field.type !== 'image' && (field.originalValue === originalValue || key === originalValue || field.label === originalValue || cleanString(field.originalValue) === cleanString(originalValue))) {
    ...
    if (Array.isArray(content.sections)) {
      ...
      if (sectionIdentifier) {
        ...
        matchedSection = content.sections.find(s => {
        ...
        // Banner fallback if targetId contains banner
        if (!matchedSection && targetId.includes('banner')) {
          matchedSection = content.sections.find(s => {
            const sId = (s.sectionId || '').toLowerCase();
            const sName = (s.sectionName || '').toLowerCase();
            return sId.includes('banner') || sName.includes('banner') || sId.includes('netanner') || sName.includes('netanner');
          });
        }
        ...
        if (matchedSection && matchedSection.fields) {
          if (isImageReq) {
            ...
            for (const [key, field] of Object.entries(matchedSection.fields)) {
              if (!field) continue;
              if (isImageMatch(field, originalValue) || key === originalValue || field.label === originalValue) {
                if (isValidImageUrl(field.value)) {
                  return field.value;
            ...
            for (const [key, field] of Object.entries(matchedSection.fields)) {
              ...
              if (field.originalValue === originalValue || key === originalValue || field.label === originalValue || cleanString(field.originalValue) === cleanString(originalValue)) {
                if (field.value !== undefined && field.value !== null && field.value !== '') {
                  return formatCmsValue(field.value);
                }
              }
      ...
      // Global fallback search across all sections ONLY if no section matched sectionIdentifier
      if (!matchedSection) {
        if (isImageReq) {
  ...
  // Return static fallback value (preserve StaticImageData object intact so Next.js Image component gets src, width, and height)
