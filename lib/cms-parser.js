...
export function isCodeOrStyleString(val) {
  ...
  // HTML Entities, Tag Fragments & Decorative Symbols / Punctuations
  ...
  // Internal route paths without spaces (e.g. /claim-your-free-seo-audit, /services)
...
export function parsePageContent(pageFilePath, slug = null) {
  ...
  function parseFile(filePath, componentName) {
    ...
    while ((match = jsxTagRegex.exec(cleanContentForTags)) !== null) {
      ...
      if (/aria-hidden=['"]true['"]/i.test(fullMatch)) {
        continue;
      }
    ...
    while ((match = imgImportRegex.exec(content)) !== null) {
      ...
      // Only parse imported images that are actually referenced in the component body
    ...
    // 6. JS Object Image Props (skip empty image paths or CSS url fragments to prevent duplicate empty inputs)
    let jsImgIdx = 1;
    ...
    const isWrapperPage = (componentName === 'Page Content' || componentName === 'page' || componentName === 'HomeClientPage' || componentName === 'HomePage') && hasSubComponents;
