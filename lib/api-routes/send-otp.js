...
export async function POST(request) {
  try {
    ...
    if (!user || !pass) {
      console.warn('SMTP credentials missing in environment variables. OTP email not sent.');
      return NextResponse.json({ error: 'Email service is currently unavailable. Please configure SMTP credentials.' }, { status: 503, headers: CORS_HEADERS });
    }
