...
// PATCH /api/applications -> Toggle isRead or mark all as read
export async function PATCH(request) {
  try {
    ...
    if (markAll) {
      ...
      return NextResponse.json({ ok: true, message: 'All applications marked as read' });
    }
    ...
    const res = await db.collection('applications').updateOne(query, {
      $set: { isRead: isRead === true }
    });

    return NextResponse.json({ ok: true, matchedCount: res.matchedCount });
  } catch (err) {
