...
export async function GET(request) {
  try {
    ...
    const category = searchParams.get('category') || 'all'; // all, career, inquiry, general
    ...
    const [allCount, unreadCount, careerCount, inquiryCount] = await Promise.all([
      threadsCol.countDocuments({ status: { $ne: 'archived' } }),
      threadsCol.countDocuments({ unreadByAdmin: true, status: { $ne: 'archived' } }),
      threadsCol.countDocuments({ category: 'career', status: { $ne: 'archived' } }),
      threadsCol.countDocuments({ category: 'inquiry', status: { $ne: 'archived' } }),
    ]);
