...
export async function GET(request) {
  try {
    // Automatically synchronize unseen Hostinger emails (with 3.5s timeout for serverless reliability)
    try {
      await Promise.race([
        triggerAutoSync({ waitForCompletion: true, limit: 25, onlyUnseen: true }),
    ...
    // Exclusively query emails received at active configured mailbox (hr@techsolutionor.com)
    ...
    const escapedMailbox = activeMailbox.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const mailboxFilter = { $or: [{ mailbox: { $regex: new RegExp(escapedMailbox, 'i') } }, { mailbox: { $exists: false } }] };
...
export async function POST(request) {
  try {
    ...
    // 2. Wrap content inside master Tech Solutionor email template (Top Logo + Center Body + Bottom Social Links)
    const finalBrandedHtml = (cleanHtml.includes('<!-- TOP: Emerald Accent Bar -->') || cleanHtml.includes('techsolutionor-logo.png'))
      ? cleanHtml
      : wrapWithTechSolutionorTemplate({
          contentHtml: cleanHtml,
          subject: cleanSubject,
          recipientName: toName || recipientEmail.split('@')[0],
        });
