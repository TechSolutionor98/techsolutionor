...
export async function GET(request) {
  try {
    ...
    if (routeObjId) {
      filter.$or = [{ _id: routeObjId }, { routeId: routeId }, { path: routeId }];
    } else if (routeId) {
    ...
    let route = null;
    if (routeObjId) {
      route = await db.collection('cms_routes').findOne({ _id: routeObjId });
    }
    if (!route && (pathParam || routeId)) {
      const p = pathParam || routeId;
      route = await db.collection('cms_routes').findOne({ path: p, websiteId });
    }
    ...
    const serviceMatch = route && route.path ? route.path.match(/^\/services\/([a-z0-9\-]+)$/) : null;
    const serviceSlug = serviceMatch ? serviceMatch[1] : (route && route.path === '/hire-us' ? 'hire-us' : null);
    ...
    if (content && Array.isArray(content.sections)) {
      ...
      if (!isHomePage) {
        for (const dbSec of content.sections) {
          ...
          if (!existsInParsed && dbSec.sectionId?.startsWith('sec_')) {
