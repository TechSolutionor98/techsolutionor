...
export async function GET() {
  try {
    ...
    const [contactUnread, appUnread, reviewUnread, commentUnread, emailUnread] = await Promise.all([
      ...
      db.collection('email_threads').countDocuments({ unreadByAdmin: true, status: { $ne: 'archived' } }).catch(() => 0),
    ]);
    ...
    const [recentContacts, recentApps, recentReviews, recentComments, recentEmails] = await Promise.all([
      ...
      db.collection('email_threads')
        .find({ unreadByAdmin: true, status: { $ne: 'archived' } })
    ...
    for (const e of recentEmails) {
      notifications.push({
        id: e._id?.toString() || e.threadId,
        _id: e._id?.toString() || e.threadId,
        type: 'email',
        category: 'inquiries',
        title: e.category === 'career' ? 'New Job Application Email' : 'New Email Message',
        author: e.applicant?.name || 'Applicant',
        email: e.applicant?.email || '',
        phone: '',
        preview: `${e.subject} • ${e.lastSnippet || 'New message in thread'}`,
        source: e.mailbox || 'hr@techsolutionor.com',
        link: `/admin/emails?threadId=${e.threadId}`,
        isRead: false,
        createdAt: e.lastMessageAt || e.createdAt || new Date().toISOString(),
      });
    }
    ...
    const totalInquiries = contactUnread + appUnread + reviewUnread + emailUnread;
  ...
  try {
    ...
    if (markAll && (!type || type === 'all')) {
      await Promise.all([
        ...
        db.collection('email_threads').updateMany({}, { $set: { unreadByAdmin: false } }),
      ]);
    ...
    if (markAll && type) {
      if (type === 'contact' || type === 'appointment' || type === 'contactMessages') {
        ...
        await db.collection('email_threads').updateMany({}, { $set: { unreadByAdmin: false } });
      }
    ...
    if (id) {
      ...
      if (type === 'contact' || type === 'appointment') {
        ...
        const res = await db.collection('email_threads').updateOne(threadQuery, { $set: { unreadByAdmin: isRead !== true, updatedAt: new Date().toISOString() } });
        updated = res.matchedCount > 0;
      } else {
            ...
            if (res3.matchedCount > 0) updated = true;
              ...
              if (res4.matchedCount > 0) updated = true;
                ...
                const res5 = await db.collection('email_threads').updateOne(
                  oid ? { $or: [{ _id: oid }, { _id: id }, { threadId: id }] } : { $or: [{ _id: id }, { threadId: id }] },
