...
export async function syncIncomingEmails({ limit = 40, markSeen = true, onlyUnseen = true } = {}) {
  ...
  await ensureEmailIndexes().catch(() => {});
    ...
    try {
      ...
      for await (const message of client.fetch(uidsToFetchFull, { source: true, uid: true, envelope: true }, { uid: isUidRange })) {
        try {
          ...
          // 6. Save message to MongoDB with deduplication protection
          try {
            await messagesCol.insertOne(newMessage);
          } catch (insertErr) {
            // E11000 duplicate key error means this message was already ingested concurrently
            if (insertErr.code === 11000) {
              console.log(`[IMAP] Duplicate message detected (${rfcMessageId}), skipping insert.`);
              if (message.uid) {
