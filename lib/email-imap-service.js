 ...
 */
export async function syncIncomingEmails({ limit = 40, markSeen = true } = {}) {
  ...
  try {
    ...
    console.log(`[IMAP] Connected! Acquiring lock on INBOX...`);
    const lock = await client.getMailboxLock('INBOX');
    console.log(`[IMAP] INBOX lock acquired. Searching...`);

    try {
      // 1. Search for unseen messages, or fallback to the latest messages
      let isUidRange = true;
      let searchRange = await client.search({ seen: false }, { uid: true });
      console.log(`[IMAP] Unseen search result:`, searchRange?.length || searchRange);

      if (!Array.isArray(searchRange) || searchRange.length === 0) {
        // Fallback: check the latest messages in INBOX
      ...
      for await (const message of client.fetch(searchRange, { source: true, uid: true, envelope: true }, { uid: isUidRange })) {
        try {
          ...
          // 8. Optionally mark message as seen on IMAP
