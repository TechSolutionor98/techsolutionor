...
export async function ensureEmailIndexes() {
  if (_indexesEnsured) return;
  try {
    const db = await getDb();
    await Promise.all([
      db.collection('email_messages').createIndex({ messageId: 1 }, { unique: true, sparse: true }).catch(() => {}),
      db.collection('email_messages').createIndex({ threadId: 1 }).catch(() => {}),
      db.collection('email_threads').createIndex({ threadId: 1 }, { unique: true, sparse: true }).catch(() => {}),
      db.collection('email_threads').createIndex({ 'applicant.email': 1, normalizedSubject: 1 }).catch(() => {}),
    ]);
    _indexesEnsured = true;
  } catch (err) {
    console.warn('ensureEmailIndexes warning:', err?.message || err);
  }
}
