...
export async function sendApplicationSubmissionConfirmation({ application }) {
  ...
  const senderEmail = (process.env.HR_EMAIL || process.env.SMTP_USER || process.env.SMTP_EMAIL || 'hr@techsolutionor.com').trim();
  const phone = application.phone || 'Not provided';
  const experience = application.experience || 'Not specified';
  const portfolio = application.portfolio || (Array.isArray(application.portfolioLinks) && application.portfolioLinks.length > 0 ? application.portfolioLinks.join(', ') : '');
  const cvCount = Array.isArray(application.cvFiles) && application.cvFiles.length > 0 ? application.cvFiles.length : (application.cv ? 1 : 0);
                  ...
                  <td style="padding: 36px 32px;">
                    ...
                    <div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 22px; margin: 0 0 26px 0;">
                      ...
                      <table width="100%" cellpadding="0" cellspacing="0" style="font-size: 13px; line-height: 1.8; color: #475569;">
                        ...
                        ${portfolio ? `
                        <tr>
                          <td style="padding: 4px 0; color: #64748b; font-weight: 600;">Portfolio / Profiles:</td>
                          <td style="padding: 4px 0; color: #0f172a; word-break: break-all;">${portfolio}</td>
                        </tr>` : ''}
                        ${cvCount > 0 ? `
                        <tr>
                          <td style="padding: 4px 0; color: #64748b; font-weight: 600;">CV / Documents:</td>
                          <td style="padding: 4px 0; color: #0f172a;">${cvCount} document(s) uploaded</td>
                        </tr>` : ''}
                        <tr>
